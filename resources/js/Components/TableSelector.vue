...
const error = ref('');
const selectFromTable = (table) => {
    if (table == 0 && props.current_view == 'bill') {
        error.value = "No hay suficiente mesas.";
        return
    }
    emit('select', table)
    isOpen.value = false
...
const filteredTables = computed(() => {
    ...
    return props.tables.filter(prd => {
    ...
    <div class="space-y-2">
        <Dialog v-model:open="isOpen">
            ...
            <DialogContent class="sm:max-w-[700px] bg-background text-foreground border-border shadow-lg">
                <DialogHeader>
                    <DialogTitle class="text-xl font-bold">Buscar Producto</DialogTitle>
                </DialogHeader>

                <div class="space-y-4">
                    <div class="relative">
                        <Search class="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" />
                        <Input v-model="manualSearch" placeholder="Buscar por nombre o código..."
                            ...
                            <Table>
                                <TableHeader class="bg-muted/50 sticky top-0 z-10">
                                    <TableRow class="border-border">
                                        <TableHead class="w-[100px]">{{t('page')}}</TableHead>
